...
func parseFlags() (options, error) {
	...
	flag.BoolVar(&o.dryRun, "dry-run", false, "run the practice loop with no window and no device, print the scoreboard and exit")
...
func dryRun(o options, cfg config.Config) error {
	...
	for i := 0; i < steps; i++ {
		...
		if u.LapDone {
			laps++
			fmt.Printf("lap %d: %.0f%% accuracy, %s -> %s\n",
	...
	fmt.Printf("\nperfect %d  good %d  miss %d  extra %d\n", st.Perfect, st.Good, st.Miss, st.Extra)
	fmt.Printf("accuracy %.0f%%  best combo x%d  %d of %d resolved\n",
		st.Accuracy*100, st.MaxCombo, st.Resolved, st.Total)

	if st.Resolved == 0 {
		return fmt.Errorf("nothing was scored; the practice loop is not running")
	}
	return nil
}
